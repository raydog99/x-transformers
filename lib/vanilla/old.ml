...
let transformer_model vs config =
  let { Config.n_embd; n_layer; n_head; layer_norm_eps; _ } = config in
  let vs = Var_store.(vs / "transformer") in
  let wte =
    Layer.embeddings
      Var_store.(vs / "wte")
      ~num_embeddings:config.Config.vocab_size
      ~embedding_dim:n_embd
  in
  let wpe =
    Layer.embeddings
      Var_store.(vs / "wpe")
      ~num_embeddings:config.Config.n_positions
      ~embedding_dim:n_embd
  in
  ...
  fun input_ids ~layer_past ~attention_mask ~token_type_ids ~position_ids ~is_training ->
    ...
    let hidden_state =
      ...
      |> Tensor.dropout ~p:config.Config.embd_p ~is_training
    in
...
let () =
  ...
  let vs = Var_store.create ~name:"transformer" ~device:(Device.cuda_if_available ()) () in
  let model = transformer_model vs config in
  
  let input_ids = Tensor.randint_low ~low:0 ~high:config.Config.vocab_size [ 2; 3; 512 ] ~options:(T Int64, Device.Cpu) in
  ...
  let output, _ = model input_ids ~layer_past ~attention_mask ~token_type_ids ~position_ids ~is_training in
